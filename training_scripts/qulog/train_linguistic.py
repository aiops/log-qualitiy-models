...
def preprocessing(features, nlp, nlp_parse, buffer=True):
    # Buffering this to prevent recalculation

    docs = get_docs(nlp_parse, features)
    ...
    if buffer and os.path.isfile(word_class_embeddings_file):
        ...
        word_class_embeddings = get_trf_embeddings(word_classs_trf_docs)
        if buffer:
...
word_class_embeddings = preprocessing(features, nlp, nlp_parse)
